...
export function Repository({ title, description, url, language, updatedAt }) {
    ...
    return (
        <a
            ...
            rel="noreferrer"
            target="_blank"
            style={isHover ? { boxShadow: `${getLangColor(language)} 0px 2px 8px 0px` } : null}
        >
            <div className={style['title_updatedAtContainer']}>
                <div className={style['title_imageContainer']}>
                    <h5 className={style.title}>
                        {title} <Image src="/git-branch.png" alt="git-branch" width="15" height="15" />
                    </h5>
                </div>
                <p className={style.updatedAt}>{new TimeAgo('en-US').format(new Date(updatedAt))}</p>
            </div>

            <p className={style.description}>{description}</p>
            <Image
                style={isHover ? { opacity: '0.7', animation: 'mover 1s infinite alternate' } : { opacity: '0.7' }}
